...
### Принцип единой ответственности(Single responsibility principle), класс Calculator был разделён на 2 класса: CalculatorInputVeiw и CalculatorOutput, в первом классе будет происходить только получение данных от пользователя, а второй же класс отвечает только за
вывод результатов.
...
### Принцип подстановки Барб интерфары Лисков(Liskov substitution principle), тот же интерфейс OperationStrategy позволяет без добавления конкретных классов, без изменения кода в классе CalculatorModel. Из - за этого класс CalculatorModel может принимать любую реализацию интерфейса OperationStrategy.
